...
<div>
    <table border="1">
        ...
        <c:forEach var="visit" items="${visits}" >
            ...
            <c:url var="newPrescription" value="/make-prescription">
                <c:param name="visitId" value="${visit.id}"/>
            </c:url>
            <c:url var="description" value="/add-description">
                <c:param name="visitId" value="${visit.id}"/>
            </c:url>
            <tr>
                <td>${visit.patient.first_name} ${visit.patient.surname}</td>
                <td>${visit.patientDescription}</td>
                <td>${visit.doctorDescription} <a href="${description}">Edytuj</a></td>
                <c:choose>
                    <c:when test="${visit.prescription != null}">
                        <td><a href="${prescription}">Recepta</a></td>
                    </c:when>
                    <c:otherwise>
                        <td><a href="${newPrescription}">Napisz Receptę</a></td>
                    </c:otherwise>
                </c:choose>
